...
const setupUIEndStr = '<p className="relative z-10 text-blue-50 font-medium text-center">Compete side-by-side!</p>\n                    </button>\n                </div>\n                \n                </div>';

const setupUIIndexStart = code.indexOf(setupUIStartStr);
let setupUIIndexEnd = code.indexOf(setupUIEndStr);
...
if (setupUIIndexStart !== -1 && setupUIIndexEnd !== -1) {
    ...
    console.log("Setup UI replaced correctly.");
} else {
    console.log("Failed to match boundaries again.");
}
