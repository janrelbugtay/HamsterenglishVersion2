...
// 1. Get QuizEditor from BubblePop
let quizEditorStartIndex = bubble.indexOf('function QuizEditor(');
let quizEditorEndIndex = bubble.lastIndexOf('function QuizEditor') === -1 ? bubble.length : bubble.length; // Actually just get from start to end of file, assuming QuizEditor is at the end.
...
quizEditorFull = quizEditorFull.replace(/"Bubble Pop Game"/g, '"Sumo Showdown Game"');

// Modify handleSave to handle isPublic
...
const handleSaveReplacement = `const [showPublishModal, setShowPublishModal] = useState(false);
  ...
  const handleSave = (isPublic: boolean) => {
    const generatedTitle = "Sumo Showdown Game";
    const validQuestions = questions.filter(q => q.text.trim());
...
let newSumo = `import React, { useState, useEffect, useCallback, useRef } from 'react';
...
// Extract SumoPlayer from original sumo
${sumo.match(/const SumoPlayer = [\s\S]*?;\n\n/)[0]}
...
export function Sumo({ onViewChange, initialGame }: { onViewChange: (view: ViewState) => void, initialGame?: any }) {
  ...
  const generateQuestion = () => {
    ...
    // Ensure we have at least two options. The GameEditor generates multiple options but Sumo only needs 2 choices
  ...
  return (
${sumo.match(/<div id="game-container" className="h-\[calc\(100vh-2rem\)\] bg-slate-800 dark:bg-black flex flex-col font-sans overflow-hidden relative"[\s\S]*?\);/)[0]}
}

${quizEditorFull}
`;

fs.writeFileSync('src/views/Sumo.tsx', newSumo);
