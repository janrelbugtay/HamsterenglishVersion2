...
const wrongReplacement = `
            wrong(b, slot) {
                ...
                // FORCE relative on slot so absolute X mark is constrained
...
code = code.replace(/wrong\(b, slot\) \{[\s\S]*?this\.checkComboEffects\(\);\s*\},/, wrongReplacement.trim() + ",");

fs.writeFileSync('public/bubble-sentence.html', code);
console.log("Patched wrong effects");
